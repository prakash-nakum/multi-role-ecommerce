...
const UpdateCategory = () => {
  ...
  const { data: categoryData, error: categoryError } = useGetCategoryQuery(id);
  const category = useMemo(() => categoryData?.data || {}, [categoryData]);

  useEffect(() => {
    if (updatedCategoryData) {
      toast.success(updatedCategoryData?.description);
    }
    if (categoryUpdatingError?.data) {
      toast.error(categoryUpdatingError?.data?.description);
    }
    if (categoryError) {
      toast.error(categoryError?.data?.description);
    }
          ...
          <div className="col-span-4">
            <div className="flex flex-col gap-4 h-full w-full relative">
              {thumbnailPreview && (
                <>
                  <Image
                    src={`${process.env.NEXT_PUBLIC_NODE_URL}/${thumbnailPreview}`}
