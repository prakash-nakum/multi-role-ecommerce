              ...
              (fetchingBrands || brands?.length === 0 ? (
                ...
                <>
                  {brands?.slice(0, 6)?.map((brand, index) => (
                    <div
                      key={index}
                      className="cursor-pointer group border p-4 rounded-primary flex flex-col gap-y-4 hover:border-black transition-colors bg-white relative"
                  ...
                  {stores?.slice(0, 6)?.map((store, index) => (
                    ...
                    >
                      <Image
                        ...
                        // src={`${process.env.NEXT_PUBLIC_NODE_URL}/${store?.thumbnail?.url}`}
