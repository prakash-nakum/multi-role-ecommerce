...
const Card = ({ index, product, ...rest }) => {
  ...
  return (
    <div
      {...rest}
      className="cursor-pointer flex-shrink-0 flex flex-col gap-y-6 group border hover:border-black transition-colors rounded-primary"
      onClick={() =>
        router.push(
    ...
    >
      <div className="relative h-[200px] w-full rounded-primary">
        ...
        <div className="flex flex-row gap-x-2.5 absolute top-4 left-4 opacity-0 group-hover:opacity-100 transition-opacity">
          <Logo
            // src={`${process.env.NEXT_PUBLIC_NODE_URL}/${product?.brand?.logo?.url}`}
